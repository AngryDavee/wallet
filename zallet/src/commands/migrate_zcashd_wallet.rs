...
impl MigrateZcashdWalletCmd {
    ...
    async fn migrate_zcashd_wallet(
        ...
        // Import view-only Sapling keys added via `z_importviewingkey`. zcashd stores
        // these as `sapextfvk` BDB records; each EFVK becomes its own view-only account,
        // matching how each spending-key entry becomes its own account above. If both
        // `z_importkey` and `z_importviewingkey` were called for the same key, the
        // spending-key path already created the account, so the `get_account_for_ufvk`
        // check below skips the duplicate.
        ...
        for (idx, zewif_extfvk) in viewing_keys.values().enumerate() {
            ...
            // `zewif-zcashd` parses Sapling types against an older `sapling-crypto`. The
            // ZIP 32 EFVK encoding is 169 bytes and stable across versions, so round-trip
            // through bytes to get the version this crate uses.
            let mut bytes = [0u8; 169];
            zewif_extfvk
                .write(&mut bytes[..])
                .expect("Sapling EFVK fits in 169 bytes");
            let extfvk = sapling::zip32::ExtendedFullViewingKey::read(&bytes[..])
                .expect("Sapling EFVK encoding is stable across sapling-crypto versions");
        ...
        // Addresses imported standalone (privkey, pubkey, or P2SH redeem script) have no
        // chain-derived exposure history, so feed them all into a single accumulator that
        // `mark_transparent_addresses_exposed` consumes below; otherwise `listaddresses`
        // (which filters on `exposed_at_height IS NOT NULL`) would never surface them.
        let mut explicit_exposures: Vec<TransparentAddress> = Vec::new();
        ...
        to_expose.extend(explicit_exposures);
